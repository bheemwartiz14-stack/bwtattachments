        ...
        <div class="grid w-full overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl dark:border-neutral-800 dark:bg-neutral-950 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <section class="flex items-center justify-center bg-slate-50 px-5 py-10 sm:px-8 lg:px-12 dark:bg-neutral-900">
                <div class="w-full max-w-md">
                    ...
                    @if ($errors->any())
                        <div class="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3.5 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-900/30 dark:text-red-300">
                            <svg class="mt-0.5 h-5 w-5 shrink-0 text-red-500 dark:text-red-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01" stroke-linecap="round"/></svg>
                    ...
                    @endif
                    @if (session('status'))
                        <div class="mt-4 flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3.5 text-sm text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-900/30 dark:text-emerald-300">
                            <svg class="mt-0.5 h-5 w-5 shrink-0 text-emerald-500 dark:text-emerald-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01" stroke-linecap="round"/></svg>
                            {{ session('status') }}
                        </div>
                    @endif
                    ...
                    <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-5">
                        ...
                        <div>
                            ...
                            <div class="relative mt-2">
                                ...
                                <input id="indenify" class="block h-12 w-full rounded-xl bg-white pl-11 pr-4 text-sm shadow-sm focus:ring-2 focus:ring-offset-0 dark:bg-neutral-950 dark:text-neutral-100 dark:placeholder-neutral-500 {{ $errors->has('indenify') ? 'border-red-300 focus:border-red-400 focus:ring-red-500 dark:border-red-700 dark:focus:border-red-500 dark:focus:ring-red-500' : 'border-slate-200 focus:border-slate-500 focus:ring-slate-500 dark:border-neutral-700 dark:focus:border-slate-400 dark:focus:ring-slate-400' }}" type="text" name="indenify" value="{{ old('indenify') }}" required autofocus autocomplete="username" placeholder="Email or username">
                            </div>
                            @error('indenify')
                        ...
                        <div>
                            ...
                            <div class="relative mt-2">
                                ...
                                <input id="password" class="block h-12 w-full rounded-xl bg-white pl-11 pr-11 text-sm shadow-sm focus:ring-2 focus:ring-offset-0 dark:bg-neutral-950 dark:text-neutral-100 dark:placeholder-neutral-500 {{ $errors->has('password') ? 'border-red-300 focus:border-red-400 focus:ring-red-500 dark:border-red-700 dark:focus:border-red-500 dark:focus:ring-red-500' : 'border-slate-200 focus:border-slate-500 focus:ring-slate-500 dark:border-neutral-700 dark:focus:border-slate-400 dark:focus:ring-slate-400' }}" type="password" name="password" required autocomplete="current-password" placeholder="Enter password">
