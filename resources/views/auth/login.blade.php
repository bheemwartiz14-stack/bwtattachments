            ...
            <section class="flex items-center justify-center bg-slate-50 px-5 py-10 sm:px-8 lg:px-12">
                <div class="w-full max-w-md">
                    ...
                    <div>
                        ...
                        <h2 class="mt-2 text-3xl font-bold tracking-tight text-slate-950">Login to your account</h2>
                        <p class="mt-2 text-sm text-slate-500">Enter your details to continue to the dashboard.</p>
                    </div>
                    @if (session('status'))
                    ...
                    <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-5">
                        @csrf
                        <div>
                            <label for="indenify" class="block text-sm font-semibold text-slate-700">Username Or Email</label>
                            <div class="relative mt-2">
                                ...
                                <input id="indenify" class="block h-12 w-full rounded-xl border-slate-200 bg-white pl-11 pr-4 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500" type="text" name="indenify" value="{{ old('indenify') }}" required autofocus autocomplete="username" placeholder="Email or username">
                            </div>
                            @error('indenify')
                        ...
                        <div>
                            ...
                            <div class="relative mt-2">
                                ...
                                <input id="password" class="block h-12 w-full rounded-xl border-slate-200 bg-white pl-11 pr-4 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500" type="password" name="password" required autocomplete="current-password" placeholder="Enter password">
                            </div>
                            @error('password')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
