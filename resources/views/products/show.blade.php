<x-layouts.public>
    ...
    <main class="min-h-screen bg-gradient-to-b from-slate-50 to-white">
        <div class="max-w-[1700px] mx-auto px-6 lg:px-8 py-6">
            ...
            <nav class="flex items-center gap-2 text-sm text-slate-500 mb-6">
                <a href="{{ route('public.home.index') }}" class="hover:text-bwtblue transition-colors no-underline">Products</a>
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                @if($product->category)
                    <a href="{{ route('public.categories.show', $product->category) }}" class="hover:text-bwtblue transition-colors no-underline">{{ $product->category->name }}</a>
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                @endif
                <span class="text-slate-900 font-medium truncate">{{ $product->product_title ?? $product->product_code }}</span>
            </nav>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-14">
                ...
                <div class="space-y-6">
                    {{-- Code & Title --}}
                    <div>
                        <span class="inline-block font-mono text-xs font-semibold tracking-widest text-bwtblue uppercase bg-blue-50 px-3 py-1 rounded-full">{{ $product->product_code ?? 'N/A' }}</span>
                        <h1 class="text-3xl lg:text-4xl font-bold text-slate-900 mt-3 leading-tight">{{ $product->product_title ?? 'Product' }}</h1>
                    </div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @if($product->category)
                                <span class="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 text-xs font-medium px-3 py-1.5 rounded-full border border-emerald-200">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z"/></svg>
                                    {{ $product->category->name }}
                                </span>
                            @endif
                            @if($product->subcategory)
                                <span class="inline-flex items-center gap-1.5 bg-blue-50 text-bwtblue text-xs font-medium px-3 py-1.5 rounded-full border border-blue-200">
                            ...
                            @if($product->connection)
                                <span class="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 text-xs font-medium px-3 py-1.5 rounded-full border border-amber-200">
                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.63a4.5 4.5 0 00-6.364-6.364L4.5 8.688a4.5 4.5 0 001.242 7.244"/></svg>
                   ...
                   @if($product->product_description)
                    ...
                    @php
                        $specs = [
                            'Weight' => $product->weight ? ['value' => $product->weight . ' kg', 'icon' => 'M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3'] : null,
                            'Width' => $product->width ? ['value' => $product->width . ' mm', 'icon' => 'M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4'] : null,
                            'Volume' => $product->volume ? ['value' => $product->volume . ' m³', 'icon' => 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4'] : null,
                            'Machine Class' => $product->machine_class ? ['value' => $product->machine_class . ' t', 'icon' => 'M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z'] : null,
                            'Cutting Edge' => $product->cutting_edge_thickness ? ['value' => $product->cutting_edge_thickness . ' mm', 'icon' => 'M6 6h12M6 12h12M6 18h12'] : null,
                            'Teeth' => $product->teeth ? ['value' => $product->teeth, 'icon' => 'M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z'] : null,
                            'Pin Hole' => $product->pin_hole ? ['value' => $product->pin_hole . ' mm', 'icon' => 'M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z'] : null,
                            'Pin Center' => $product->pin_center ? ['value' => $product->pin_center . ' mm', 'icon' => 'M6.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0zM12.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0zM18.75 12a.75.75 0 11-1.5 0 .75.75 0 011.5 0z'] : null,
                            'Stick Width' => $product->stick_width ? ['value' => $product->stick_width . ' mm', 'icon' => 'M6 18L18 6M6 6l12 12'] : null,
                        ];
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                @foreach($specs as $label => $spec)
                                    <div class="bg-slate-50 rounded-xl border border-slate-200 p-4 transition-all duration-200 hover:border-bwtblue/30 hover:shadow-sm hover:bg-white">
    ...
    <div id="lightbox"
        ...
        <img id="lightboxImage" src="" alt="" class="max-w-[90vw] max-h-[90vh] object-contain rounded-2xl">
    </div>
