...
<x-layouts.app>
    <x-slot:title>{{ $isEdit ? 'Edit' : 'Add' }} Retailer User - Attachment Portal</x-slot:title>
    <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('client.dashboard')], ['label' => 'Retailer Users', 'url' => route('client.retailer-users.index')], ['label' => $isEdit ? 'Edit' : 'New']]" />

    <div class="space-y-4">
        <x-ui.hero title="{{ $isEdit ? 'Edit' : 'Add' }} Retailer User" subtitle="{{ $isEdit ? 'Update retailer user details and settings' : 'Create a new retailer user account' }}">
            <x-slot:icon>
        ...
        <form action="{{ $isEdit ? route('client.retailer-users.update', $user) : route('client.retailer-users.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
                    ...
                    <div class="grid grid-cols-1 gap-x-8 gap-y-6 lg:grid-cols-2">
                        ...
                        <div class="lg:col-span-2">
                            <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-3">
                                ...
                                <div>
                                    <input type="hidden" name="roles" value="{{ $userRole ?? 'Retailer' }}">
                                    ...
                                    <div class="inline-flex items-center gap-2.5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm dark:border-emerald-800 dark:bg-emerald-900/20">
                                        ...
                                        <span class="font-semibold text-emerald-800 dark:text-emerald-300">{{ $userRole ?? 'Retailer' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
