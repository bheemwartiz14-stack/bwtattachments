                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-neutral-800">
                            @forelse($products ?? [] as $product)
                                <tr class="transition-colors hover:bg-rose-50 dark:hover:bg-neutral-900/50">
                                    ...
                                    @php $userPrice = $product->productPrices->firstWhere('user_id', auth()->id()); @endphp
                                    <td class="px-6 py-4 text-sm font-semibold text-black dark:text-neutral-100">{{ config('app.currency_symbol') }}{{ number_format($userPrice?->price ?? $product->ddp_price, 2) }}</td>
