@php
    $isRetailer = auth()->user()->hasRole('Retailer');
    $prefix = $isRetailer ? 'retailer' : 'client';
@endphp

<x-layouts.app>
    <x-slot:title>Profile Settings - Attachment Portal</x-slot:title>
    <x-breadcrumb :items="[['label' => $isRetailer ? 'Retailer Portal' : 'Client Portal', 'url' => route($prefix . '.dashboard')], ['label' => 'Profile Settings']]" />
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
            <h2 class="text-base font-semibold text-slate-950 dark:text-neutral-100 mb-6">Profile Information</h2>
            <form method="POST" action="{{ route($prefix . '.profile.update') }}" class="space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label for="name" class="block text-sm font-medium text-slate-700 dark:text-neutral-300 mb-1.5">Name</label>
        ...
        <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
            <h2 class="text-base font-semibold text-slate-950 dark:text-neutral-100 mb-6">Change Password</h2>
            <form method="POST" action="{{ route($prefix . '.profile.password') }}" class="space-y-5">
                @csrf
                @method('PUT')
