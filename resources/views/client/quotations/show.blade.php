<x-layouts.app>
    <x-slot:title>{{ $quotation->quotation_number }} - Attachment Portal</x-slot:title>
    ...
    <div class="space-y-6">
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                <div>
                    <h2 class="text-sm font-mono font-semibold text-black dark:text-gray-100">{{ $quotation->quotation_number }}</h2>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Created {{ $quotation->created_at->format('F d, Y') }}</p>
                </div>
                <div class="flex items-center gap-2">
                    @php
                        $statusClasses = [
            ...
            <div class="divide-y divide-slate-100 dark:divide-slate-700">
                @forelse($quotation->items as $item)
                    <div class="px-6 py-4 hover:bg-rose-50 dark:hover:bg-slate-800/50 transition-colors">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between">
                                    <div>
                                        <p class="text-sm font-medium text-black dark:text-gray-100">{{ $item->product->description ?? $item->description }}</p>
                                        <p class="text-xs font-mono text-gray-400 dark:text-gray-500 mt-0.5">{{ $item->product->product_code ?? $item->product_code }}</p>
                                    </div>
                                </div>
                                <div class="mt-2 grid grid-cols-3 gap-4 text-xs text-gray-700 dark:text-gray-400">
                                    <span>Quantity: <span class="font-medium text-black dark:text-gray-100">{{ $item->quantity }}</span></span>
                                    <span>Unit Price: <span class="font-medium text-black dark:text-gray-100">${{ number_format($item->unit_price, 2) }}</span></span>
                                    <span>Total: <span class="font-medium text-emerald-600 dark:text-emerald-400">${{ number_format($item->total, 2) }}</span></span>
                                </div>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="mt-3 text-sm text-gray-400 dark:text-gray-500">No items in this quotation</p>
                    </div>
                @endforelse
            </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-700 bg-rose-50 dark:bg-slate-800/50">
                <div class="max-w-sm ml-auto space-y-2">
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="font-medium text-black dark:text-gray-100">${{ number_format($quotation->subtotal, 2) }}</span>
                    </div>
                    @if($quotation->margin > 0)
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-700 dark:text-gray-400">Margin ({{ $quotation->margin }}%)</span>
                            <span class="font-medium text-emerald-600 dark:text-emerald-400">+${{ number_format($quotation->margin_amount, 2) }}</span>
                        </div>
                    @endif
                    <div class="flex items-center justify-between text-lg font-bold pt-2 border-t border-slate-100 dark:border-slate-700">
                        ...
                        <span class="text-black dark:text-gray-100">${{ number_format($quotation->total, 2) }}</span>
                    </div>
                </div>
            </div>
            ...
            @if($quotation->notes)
                <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-700">
                    <p class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Notes</p>
                    <p class="text-sm text-gray-700 dark:text-gray-400">{{ $quotation->notes }}</p>
                </div>
            @endif
