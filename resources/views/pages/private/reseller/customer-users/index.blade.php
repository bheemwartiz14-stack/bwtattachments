<x-layouts.app>
    ...
    <div class="space-y-6">
        <x-ui.hero title="Manage Customers" subtitle="">
            <x-slot:actions>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 dark:border-neutral-800">
                <form method="GET" action="{{ route('reseller.customer-users.index') }}" class="flex flex-1 items-center gap-3">
                    ...
                    @if(request('search'))
                        <a href="{{ route('reseller.customer-users.index') }}" wire:navigate class="text-sm font-medium text-slate-500 hover:text-slate-700 dark:text-neutral-400">Clear</a>
                    @endif
                </form>
            </div>
                            ...
                            <tr class="transition-colors hover:bg-slate-50/80 dark:hover:bg-neutral-800/30">
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-3">
                                          <a href="{{ route('reseller.customer-users.show', $user) }}" wire:navigate title="View"
                                        ...
                                        <a href="{{ route('reseller.customer-users.show', $user) }}" wire:navigate
                                ...
                                <td class="px-5 py-4 text-right">
                                        <div class="flex items-center justify-end gap-1">
                                            ...
                                            <a href="{{ route('reseller.customer-users.edit', $user) }}" wire:navigate title="Edit"
                                                class="inline-flex items-center justify-center rounded-lg p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500 dark:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-white">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                                            </a>
                                            <form action="{{ route('reseller.customer-users.destroy', $user) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this user?')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="flex flex-col items-center justify-center px-5 py-16 text-center">
                                        ...
                                        <h3 class="mt-4 text-sm font-semibold text-slate-900 dark:text-white">No  Customer found</h3>
                                        <a href="{{ route('reseller.customer-users.create') }}" wire:navigate class="mt-6 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:bg-emerald-600 dark:hover:bg-emerald-700">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
                                            Add  Customer
                                        </a>
                                    </div>
