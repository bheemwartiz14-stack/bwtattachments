...
<x-layouts.app>
    ...
    <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('reseller.dashboard')], ['label' => 'Customer Users', 'url' => route('client.reseller-users.index')], ['label' => $isEdit ? 'Edit' : 'New']]" />
    <div class="space-y-4">
        <x-ui.hero title="{{ $isEdit ? 'Edit' : 'Add' }} Customer" subtitle="{{ $isEdit ? 'Update Customer' : 'Create a new Customer' }}">
            <x-slot:icon>
        ...
        <form action="{{ $isEdit ? route('reseller.customer-users.update', $user) : route('reseller.customer-users.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
