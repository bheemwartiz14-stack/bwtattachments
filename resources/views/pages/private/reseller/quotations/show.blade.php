<x-layouts.app>
    <x-slot:title>{{ $quotation->quotation_number }} - {{ $siteTitle }}</x-slot:title>
    <x-breadcrumb :items="[
        ['label' => 'Reseller Portal', 'url' => route('client.dashboard')],
    ...
    <div class="space-y-6">
        <x-ui.hero title="Quotation {{ $quotation->quotation_number }}" icon="heroicon-o-document-text"
                ...
                <x-slot:actions>
                    <form action="{{ route('reseller.quotations.download', $quotation) }}" method="GET" class="inline">
            ...
            <div
                ...
                <div
                    class="px-6 py-4 border-b border-slate-100 dark:border-neutral-800 flex items-center justify-between">
                    <div>
                        <h2 class="text-sm font-mono font-semibold text-black dark:text-neutral-100">
                            {{ $quotation->quotation_number }}</h2>
                        <p class="text-xs text-gray-400 dark:text-neutral-500 mt-0.5">Created
                            {{ $quotation->created_at->format('M d, Y') }}</p>
                    </div>
                        ...
                        <div class="px-6 py-4 hover:bg-rose-50 dark:hover:bg-neutral-900/50 transition-colors">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div
                                        class="mt-2 grid grid-cols-3 gap-4 text-xs text-gray-700 dark:text-neutral-400">
                                        <span>Quantity: <span
                ...
                @endphp
                <div
                    class="px-6 py-4 border-t border-slate-100 dark:border-neutral-800 bg-rose-50 dark:bg-neutral-900/50">
                    <div class="max-w-sm ml-auto space-y-2">
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-700 dark:text-neutral-400">Subtotal</span>
                            <span
                                class="font-medium text-black dark:text-neutral-100">{{ config('app.currency_symbol') }}
                                {{ $quotation->sub_total }}</span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-700 dark:text-neutral-400">VAT ({{ $taxRate }}%)</span>
                            <span
                                class="font-medium text-amber-600 dark:text-amber-400">{{ config('app.currency_symbol') }}{{ $quotation->tax_amount }}</span>
                        </div>
                        ...
                        <div
                            ...
                            <span
                                class="text-black dark:text-neutral-100">{{ config('app.currency_symbol') }}{{ $quotation->grand_total }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div
                    class="px-6 py-4 border-t border-slate-100 dark:border-neutral-800 flex items-center justify-between">
                    <p class="text-xs text-gray-400 dark:text-neutral-500">
                        {{ $quotation->items->count() }} item(s) &middot; Generated on
                        {{ $quotation->created_at->format('M d, Y') }}
                    </p>
                    <form action="{{ route('reseller.quotations.download', $quotation) }}" method="GET"
                        class="inline">
                        <button type="submit"
                            class="inline-flex items-center justify-center gap-2 rounded-lg bg-rose-50 px-4 py-2 text-sm font-medium text-rose-700 transition-colors hover:bg-rose-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:bg-rose-900/30 dark:text-rose-300 dark:hover:bg-rose-900/50">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
