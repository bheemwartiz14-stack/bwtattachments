<x-layouts.app>
    <x-slot:title>My Quotations - {{ $siteTitle }}</x-slot:title>
    <x-breadcrumb :items="[['label' => 'Reseller Portal', 'url' => route('reseller.dashboard')], ['label' => 'My Quotations']]" />
    ...
    <div class="space-y-6">
        <x-ui.hero title="My Quotations" icon="heroicon-o-document-text">
            <x-slot:actions>
                <a href="{{ route('reseller.quotations.create') }}" wire:navigate
        ...
        <div
            class="overflow-hidden rounded-xl border border-slate-100 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-neutral-800">
                        @forelse($quotations ?? [] as $quotation)
                            <tr class="transition-colors hover:bg-rose-50 dark:hover:bg-neutral-900/50">
                                <td class="px-6 py-4">
                                    <a href="{{ route('reseller.quotations.show', $quotation) }}" wire:navigate
                                        class="font-mono text-sm font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">{{ $quotation->quotation_number }}</a>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-700 dark:text-neutral-400">
                                    {{ $quotation->created_at->format('M d, Y') }}</td>
                                <td class="px-6 py-4 text-sm text-gray-700 dark:text-neutral-400">
                                    {{ $quotation->items_count ?? $quotation->items->count() }}</td>
                                <td class="px-6 py-4 text-sm font-semibold text-black dark:text-neutral-100">
                                    {{ config('app.currency_symbol') }}{{ $quotation->grand_total }}</td>
                                <td class="px-6 py-4">
                                    @php
                                        $statusClasses = [
                                            'draft' =>
                                                'bg-slate-100 text-slate-800 dark:bg-neutral-900 dark:text-neutral-300',
                                            'sent' =>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form action="{{ route('reseller.quotations.download', $quotation) }}"
                                            method="GET" class="inline">
                                            <button type="submit"
                                                class=" cursor-pointer inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-black shadow-sm transition-colors hover:bg-rose-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100 dark:hover:bg-neutral-800">
                                                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor" stroke-width="2">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <form method="POST"
                                            action="{{ route('reseller.quotations.send-email', $quotation) }}"
                                            class="inline">
                                            @csrf
                                          <button type="submit"
                                                class="cursor-pointer inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-black shadow-sm transition-colors hover:bg-rose-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100 dark:hover:bg-neutral-800">
                                        ...
                                        <form method="POST"
                                            action="{{ route('reseller.quotations.destroy', $quotation) }}"
                                            class="inline"
                                            onsubmit="return confirm('Delete quotation {{ $quotation->quotation_number }}? This cannot be undone.')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="cursor-pointer inline-flex items-center justify-center gap-2 rounded-lg border border-red-200 bg-white px-3 py-1.5 text-xs font-medium text-red-600 shadow-sm transition-colors hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:border-red-900/50 dark:bg-neutral-900 dark:text-red-400 dark:hover:bg-red-900/20">
                                                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor" stroke-width="2">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
