<x-layouts.app>
    ...
    <x-breadcrumb :items="[['label' => 'Wholesaler Portal', 'url' => route('client.dashboard')], ['label' => 'My Orders']]" />
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-neutral-800">
                        @forelse($orders ?? [] as $quotation)
                            ...
                            <tr class="transition-colors hover:bg-rose-50 dark:hover:bg-neutral-900/50">
                                <td class="px-6 py-4">
                                    <a href="{{ route('client.orders.show', $quotation) }}" wire:navigate
                                        class="font-mono text-sm font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">{{ $quotation->order_number }}</a>
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm font-semibold text-black dark:text-neutral-100">
                                    {{ config('app.currency_symbol') }}{{ number_format((float) str_replace(',', '', (string) $rawGrand), 2) }}
                                </td>
                                <td class="px-6 py-4">
                                    @php
                                        $statusClasses = [
                                        ...
                                        $statusValue =
                                            $quotation->status instanceof \BackedEnum
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form action="{{ route('client.orders.download', $quotation) }}" method="GET"
                                            class="inline">
                                            <button type="submit"
                                        ...
                                        <form method="POST"
                                            action="{{ route('client.orders.send-email', $quotation) }}"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                                                class="cursor-pointer inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-black shadow-sm transition-colors hover:bg-rose-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100 dark:hover:bg-neutral-800">
