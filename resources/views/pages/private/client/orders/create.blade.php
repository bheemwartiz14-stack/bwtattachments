<x-layouts.app>
    ...
    <x-breadcrumb :items="[
        ...
        ['label' => 'Orders', 'url' => route('client.order.index')],
        ['label' => 'Create Order'],
    ]" />
    ...
    <form id="quotation-form" action="{{ route('client.quotations.store') }}" method="POST" class="space-y-6">
            ...
            <section
                ...
                <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-4 dark:border-neutral-800">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-neutral-400">Administrator contact details</p>
                    </div>
                </div>
                <div class="grid grid-cols-1 gap-4 p-6 sm:grid-cols-2">
                    <div
                        class="rounded-lg border border-slate-100 bg-slate-50/50 px-4 py-3 dark:border-neutral-800 dark:bg-neutral-900/50">
                        <p class="text-xs font-medium text-slate-400">Name</p>
                        <p class="mt-1 text-sm font-medium text-slate-700 dark:text-neutral-300">
                            {{ $admin?->name ?? '—' }}</p>
                    </div>
                    <div
                        class="rounded-lg border border-slate-100 bg-slate-50/50 px-4 py-3 dark:border-neutral-800 dark:bg-neutral-900/50">
                        <p class="text-xs font-medium text-slate-400">Email</p>
                        <p class="mt-1 text-sm text-slate-700 dark:text-neutral-300">{{ $admin?->email ?? '—' }}</p>
                    </div>
                    <div
                        class="rounded-lg border border-slate-100 bg-slate-50/50 px-4 py-3 dark:border-neutral-800 dark:bg-neutral-900/50">
                        <p class="text-xs font-medium text-slate-400">Phone</p>
                        <p class="mt-1 text-sm text-slate-700 dark:text-neutral-300">{{ $admin?->phone ?? '—' }}</p>
                    </div>
                    <div
                        class="rounded-lg border border-slate-100 bg-slate-50/50 px-4 py-3 dark:border-neutral-800 dark:bg-neutral-900/50">
                        <p class="text-xs font-medium text-slate-400">Role</p>
                        <p class="mt-1 text-sm text-slate-700 dark:text-neutral-300">Administrator</p>
                    </div>
                </div>
            </section>
            ...
            <div
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <x-forms.input name="order_date" label="Order Date" type="date" :value="$orderDate->format('Y-m-d')" />
                    </div>
                </div>
            </div>
            <div
        ...
        <div
            ...
            <div class="p-6">
                <x-forms.textarea name="quotation_email_message" label="Quotation Email Message"
                    placeholder="Enter the message that will be included in the email sent with this quotation."
                    :value="old('quotation_email_message')" rows="4" />
            </div>
        </div>
