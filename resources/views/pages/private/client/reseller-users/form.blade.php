...
<x-layouts.app>
    <x-slot:title>{{ $isEdit ? 'Edit' : 'Add' }} Reseller User - BWT</x-slot:title>
    <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('client.dashboard')], ['label' => 'Reseller Users', 'url' => route('client.reseller-users.index')], ['label' => $isEdit ? 'Edit' : 'New']]" />

    <div class="space-y-4">
        <x-ui.hero title="{{ $isEdit ? 'Edit' : 'Add' }} Reseller User" subtitle="{{ $isEdit ? 'Update Reseller user details and settings' : 'Create a new Reseller user account' }}">
            <x-slot:icon>
        ...
        <form action="{{ $isEdit ? route('client.reseller-users.update', $user) : route('client.reseller-users.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
                            ...
                            <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-3">
                                ...
                                <div>
                                    ...
                                    <div class="inline-flex items-center gap-2.5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm dark:border-emerald-800 dark:bg-emerald-900/20">
                                        ...
                                        <span class="font-semibold text-emerald-800 dark:text-emerald-300">{{ $userRole ?? 'Reseller' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md dark:border-neutral-800 dark:bg-neutral-900">
                <div class="flex items-center gap-3 border-b border-slate-100 px-8 py-6 dark:border-neutral-800">
                    ...
                    <h2 class="text-base font-semibold text-slate-900 dark:text-white">Reseller Commission Settings</h2>
                </div>
                <div class="p-8">
                    <div class="grid grid-cols-1 gap-x-8 gap-y-6 lg:grid-cols-2">
                        <x-forms.input
