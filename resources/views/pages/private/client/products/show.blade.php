<x-layouts.app>
    ...
    <div class="space-y-6">
        {{-- HERO --}}
        <div
            class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 shadow-2xl">
            <div
                ...
                <div class="shrink-0 w-full md:w-80 md:h-auto bg-slate-800/50">
                    ...
                    @else
                        <div class="flex h-full w-full items-center justify-center">
                            <svg class="h-20 w-20 text-slate-600" fill="none" viewBox="0 0 24 24"
                ...
                <div class="flex-1 p-8 flex flex-col justify-between">
                    <div>
                        <div class="flex items-start justify-between">
                            <div>
                                <h1 class="text-3xl font-bold tracking-tight text-white">{{ $product->product_title }}
                                </h1>
                            ...
                            <div class="flex items-center gap-2">
                                @if ($product->status !== null)
                                    <span
                                        class="inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-semibold shadow-sm {{ $product->status ? 'bg-emerald-500/20 text-emerald-300 ring-1 ring-emerald-500/30' : 'bg-red-500/20 text-red-300 ring-1 ring-red-500/30' }}">
                                        <span
                                            class="h-1.5 w-1.5 rounded-full {{ $product->status ? 'bg-emerald-400' : 'bg-red-400' }}"></span>
                        ...
                        @if ($product->category || $product->subcategory)
                            <div class="mt-4 flex flex-wrap gap-2">
                                @if ($product->category)
                                    <span
                                        class="inline-flex items-center rounded-lg bg-white/10 px-3 py-1.5 text-xs font-medium text-white/80 ring-1 ring-white/20">{{ $product->category->name }}</span>
                                @endif
                                @if ($product->subcategory)
                                    <span
                                        class="inline-flex items-center rounded-lg bg-white/10 px-3 py-1.5 text-xs font-medium text-white/80 ring-1 ring-white/20">{{ $product->subcategory->name }}</span>
                                @endif
                                @if ($product->connection)
                                    <span
                                        class="inline-flex items-center rounded-lg bg-white/10 px-3 py-1.5 text-xs font-medium text-white/80 ring-1 ring-white/20">{{ $product->connection->name }}</span>
                                @endif
                            </div>
                        @endif
                    </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    <div
                        ...
                        <p class="mt-1.5 text-2xl font-bold text-slate-900 dark:text-white">
                            {{ $product->weight ?? '' }} <span class="text-sm font-normal text-slate-400">kg</span></p>
                    </div>
                    <div
                        class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                    ...
                    <div
                        ...
                        <p class="mt-1.5 text-2xl font-bold text-slate-900 dark:text-white">
                            {{ $product->machine_class ?? '' }} <span
                                class="text-sm font-normal text-slate-400">t</span></p>
                    </div>
                </div>
                            ...
                            <div
                                ...
                                <dd class="mt-1 text-lg font-semibold text-slate-900 dark:text-white">
                                    @if ($value !== null && $value !== '')
                                        ...
                                        @if ($config['unit'])
                                            <span
                                                class="text-sm font-normal text-slate-400">{{ $config['unit'] }}</span>
                                        @endif
                                    @else
            ...
            <div class="space-y-6">
                ...
                <div
                    ...
                    <div class="mb-4 flex items-center gap-2.5 border-b border-slate-100 pb-4 dark:border-neutral-800">
                        ...
                        <span
                            class="ml-auto rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-500 dark:bg-neutral-800 dark:text-neutral-300">{{ $allImages->count() }}</span>
                    </div>
                    ...
                    @if ($allImages->count() > 0)
                        <div class="space-y-4">
                            <div
                                ...
                                <div class="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity cursor-pointer"
                                    onclick="document.getElementById('adminMainImage').click()">
                    ...
                    @else
                        <div class="py-8 text-center">
                            <svg class="mx-auto h-12 w-12 text-slate-300 dark:text-neutral-600" fill="none"
    ...
    <script>
        ...
        function openAdminLightbox(src) {
            ...
            overlay.innerHTML =
                '<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onclick="closeAdminLightbox(event)">' +
