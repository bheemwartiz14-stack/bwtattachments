<x-layouts.app>
    @php
        $logoMedia =
        $user->getFirstMedia('customer_logo') ?: $user->userMeta?->getFirstMedia('customer_logo');
    ...
    <x-breadcrumb :items="[
        ['label' => 'Customer Portal', 'url' => route('customer.dashboard')],
    ...
    @endif
    <form id="order-form" action="{{ route('customer.orders.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <input type="hidden" id="order_to_user_id" name="order_to_user_id" value="{{ $resalleruser->id }}">
            ...
            <div
                ...
                <div class="p-6">
                    <div class="max-w-xl space-y-6">
                        ...
                        {{-- Company Logo --}}
                        <div>
                            <x-forms.image-dropzone name="customer_logo" :existingImageUrl="$logoUrl" :existingImageId="$logoId"
                                label="Customer Company Logo" accept="image/jpeg,image/png,image/webp"
            ...
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col dark:border-neutral-800 dark:bg-neutral-950">
                <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-4 dark:border-neutral-800">
                    ...
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-slate-500 dark:text-neutral-400">Resaller  notification recipient</p>
                    </div>
                </div>
                <div class="p-6 flex-1">
                    <div class="flex items-center gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-neutral-800 dark:bg-neutral-900/60">
                        <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm font-bold text-blue-700 ring-4 ring-white dark:bg-blue-900/50 dark:text-blue-400 dark:ring-neutral-950">
                            {{ strtoupper(substr($resalleruser?->name ?? '?', 0, 1)) }}
                        </div>
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2">
                                <p class="truncate text-sm font-semibold text-slate-900 dark:text-white">{{ $resalleruser?->name ?? 'Unknown User' }}</p>
                            </div>
                            <p class="mt-1 truncate text-xs text-slate-500 dark:text-neutral-400">{{ $resalleruser?->email ?? 'No email available' }}</p>
                            @if ($resalleruser?->phone)
                                <p class="mt-0.5 text-xs text-slate-500 dark:text-neutral-400">{{ $resalleruser->phone }}</p>
                            @endif
                        </div>
