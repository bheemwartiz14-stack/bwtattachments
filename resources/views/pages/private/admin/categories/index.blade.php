<x-layouts.app>
    ...
    <x-breadcrumb :items="[['label' => 'Admin Portal', 'url' => route('admin.dashboard')], ['label' => 'Categories']]" />

    <div class="space-y-6">
        <x-ui.hero title="Categories" subtitle="Manage categories and subcategories">
            <x-slot:actions>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <div class="border-b border-slate-100 px-5 py-4 dark:border-neutral-800">
                <h2 class="text-base font-semibold text-slate-900 dark:text-white">All Categories</h2>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50/50 dark:border-neutral-800 dark:bg-neutral-900/50">
                            ...
                            <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-neutral-400">Products</th>
                            <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-neutral-400">Subcategories</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-neutral-800">
                        @forelse($categories ?? [] as $category)
                            <tr class="transition-colors hover:bg-slate-50/80 dark:hover:bg-neutral-800/30">
                                ...
                                <td class="px-5 py-4">
                                    <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-700 dark:bg-neutral-800 dark:text-neutral-300">{{ $category->products_count ?? 0 }}</span>
                                </td>
                                <td class="px-5 py-4">
                                    <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-700 dark:bg-neutral-800 dark:text-neutral-300">{{ $category->subcategories_count ?? $category->subcategories->count() ?? 0 }}</span>
                                </td>
                                <td class="px-5 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('admin.categories.edit', $category) }}"  wire:navigate title="Edit"
                            ...
                            <tr>
                                <td colspan="4">
                                    <div class="flex flex-col items-center justify-center px-5 py-16 text-center">
                                        ...
                                        <h3 class="mt-4 text-sm font-semibold text-slate-900 dark:text-white">No Categories</h3>
                                        <p class="mt-1 text-sm text-slate-500 dark:text-neutral-400">Get started by creating a new category.</p>
                                        <a href="{{ route('admin.categories.create') }}" wire:navigate class="mt-6 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:bg-emerald-600 dark:hover:bg-emerald-700">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
                                            Add Category
                                        </a>
                                    </div>
