        ...
        @if ($errors->any())
            <div
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-red-800 dark:text-red-300">Please fix
        ...
        <form action="{{ $isEdit ? route('admin.products.update', $product) : route('admin.products.store') }}"
            ...
            <div
                ...
                <div class="p-8 space-y-5">
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <x-forms.select name="subcategory_id" label="Subcategory" required :options="[]"
                            placeholder="Select Category first" disabled
                            data-selected="{{ old('subcategory_id', $product->subcategory_id ?? '') }}" />
                        <x-forms.select name="connection_id" label="Connection" required :options="$connectionTypes ?? []"
                            :value="$product->connection_id ?? ''" placeholder="Select Connection" />
                        <x-forms.input name="drawing_number" label="Drawing Number" required :value="$product->drawing_number ?? ''"
                            placeholder="Enter drawing number" />
                    </div>
                    <x-forms.trix name="product_description" label="Product Description"
                        description="Add product details and specifications" required :value="$product->product_description ?? ''"
            ...
            <div
                ...
                <div class="p-8 space-y-6">
                    <x-forms.image-dropzone name="product_feature_image" :existingImageUrl="$isEdit ? $product->getFirstMediaUrl('images') : null" :existingImageId="$isEdit && $product->getFirstMedia('images')
                        ? $product->getFirstMedia('images')->id
                        : null"
                        label="Product Feature Image" />
                    <x-forms.gallery-dropzone :existingImages="$isEdit
                        ? $product
                            ->getMedia('gallery')
                            ->map(fn($m) => ['url' => $m->getUrl(), 'name' => $m->file_name, 'id' => $m->id])
                            ->toArray()
                        : []" name="product_gallery_images" />
                    <x-forms.pdf-upload :existingFile="$isEdit && $product->getFirstMedia('pdfs')
                        ? $product->getFirstMedia('pdfs')->file_name
                        : null" :existingUrl="$isEdit && $product->getFirstMedia('pdfs') ? $pdfUrl : null" :existingSize="$isEdit && $product->getFirstMedia('pdfs')
                        ? $product->getFirstMedia('pdfs')->size
                        : null" :existingFileId="$isEdit && $product->getFirstMedia('pdfs') ? $product->getFirstMedia('pdfs')->id : null"
                        name="product_pdf" />
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3" x-data="{ submitting: false }">
                ...
                <x-ui.button type="submit" :loading="false" x-bind:loading="submitting"
                    x-on:click="setTimeout(() => submitting = true, 50)" :label="$isEdit ? 'Update Product' : 'Create Product'">
                    <x-slot:icon>
