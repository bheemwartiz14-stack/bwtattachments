<x-layouts.app>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <x-stat-card label="Active Wholesalers" :value="number_format($stats['active_clients'] ?? 0)" trend="up" :trendText="'+' . ($stats['new_clients'] ?? 0) . ' this month'"
            ...
            {{-- <x-stat-card label="Total Orders" :value="number_format($stats['total_order'] ?? 0)" trend="up" :trendText="($stats['weekly_quotations'] ?? 0) . ' this week'"
        ...
        <div class="">
            <div
                class="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <a href="{{ route('admin.products.index') }}" wire:navigate
                        class="flex items-center gap-4 rounded-xl border border-slate-100 p-4 transition-all hover:border-emerald-200 hover:bg-emerald-50 hover:shadow-sm dark:border-neutral-800 dark:bg-neutral-900/50 dark:hover:border-emerald-800 dark:hover:bg-emerald-900/20">
                        <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">{{ number_format($stats['total_products'] ?? 0) }} items
                            </p>
                        </div>
                    </a>
                    <a href="{{ route('admin.categories.index') }}" wire:navigate
                        class="flex items-center gap-4 rounded-xl border border-slate-100 p-4 transition-all hover:border-blue-200 hover:bg-blue-50 hover:shadow-sm dark:border-neutral-800 dark:bg-neutral-900/50 dark:hover:border-blue-800 dark:hover:bg-blue-900/20">
                        <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">{{ number_format($stats['total_categories'] ?? 0) }}
                                categories</p>
                        </div>
                    </a>
                    <a href="{{ route('admin.subcategories.index') }}" wire:navigate
                        class="flex items-center gap-4 rounded-xl border border-slate-100 p-4 transition-all hover:border-indigo-200 hover:bg-indigo-50 hover:shadow-sm dark:border-neutral-800 dark:bg-neutral-900/50 dark:hover:border-indigo-800 dark:hover:bg-indigo-900/20">
                        <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">{{ number_format($stats['total_subcategories'] ?? 0) }}
                                total</p>
                        </div>
                    </a>
                    <a href="{{ route('admin.connections.index') }}" wire:navigate
                        class="flex items-center gap-4 rounded-xl border border-slate-100 p-4 transition-all hover:border-teal-200 hover:bg-teal-50 hover:shadow-sm dark:border-neutral-800 dark:bg-neutral-900/50 dark:hover:border-teal-800 dark:hover:bg-teal-900/20">
                        <div
                            class="flex h-12 w-12 items-center justify-center rounded-xl bg-teal-100 dark:bg-teal-900/30">
                            <svg class="w-6 h-6 text-teal-600 dark:text-teal-400" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <a href="{{ route('admin.wholeseller.index') }}" wire:navigate
                        class="flex items-center gap-4 rounded-xl border border-slate-100 p-4 transition-all hover:border-amber-200 hover:bg-amber-50 hover:shadow-sm dark:border-neutral-800 dark:bg-neutral-900/50 dark:hover:border-amber-800 dark:hover:bg-amber-900/20">
                        <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">{{ number_format($stats['total_users'] ?? 0) }} accounts
                            </p>
                        </div>
                    </a>
                </div>
            ...
            <div
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('admin.products.index') }}"
                        class="text-sm font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">View
                        All</a>
                </div>
                    ...
                    <div class="space-y-2">
                        @foreach ($recentProducts as $product)
                            <a href="{{ route('admin.products.show', $product) }}" wire:navigate
                                class="flex items-center justify-between rounded-xl p-3 transition-colors hover:bg-slate-50 dark:hover:bg-neutral-800/50">
                                <div class="flex items-center gap-3 min-w-0">
                                    <div
                                        class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-neutral-800">
                                    ...
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-medium text-slate-900 dark:text-white">
                                            {{ $product->product_title ?? $product->product_description }}</p>
                                        <p class="text-xs text-slate-500">{{ $product->product_code }}</p>
                                    </div>
                                </div>
                                <span
                                    class="shrink-0 text-xs text-slate-400">{{ $product->created_at->diffForHumans() }}</span>
                            </a>
                        @endforeach
                    </div>
                @else
