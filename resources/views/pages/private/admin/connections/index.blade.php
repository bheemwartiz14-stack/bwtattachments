<x-layouts.app>
    ...
    <div class="space-y-6">
        <x-ui.hero title="Manage Connection" icon="heroicon-o-link">
            <x-slot:actions>
                <a href="{{ route('admin.connections.create') }}" wire:navigate
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <div class="border-b border-slate-100 px-5 py-4 dark:border-neutral-800">
                <h2 class="text-base font-semibold text-slate-900 dark:text-white">All Connections</h2>
            </div>
                            ...
                            <tr class="transition-colors hover:bg-slate-50/80 dark:hover:bg-neutral-800/30">
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-100 dark:bg-amber-900/30">
                                            <svg class="h-4 w-4 text-amber-600 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                                        </div>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('admin.connections.edit', $connection) }}" wire:navigate title="Edit" 
                            ...
                            <tr>
                                <td colspan="4">
                                    <div class="flex flex-col items-center justify-center px-5 py-16 text-center">
                                        ...
                                        <h3 class="mt-4 text-sm font-semibold text-slate-900 dark:text-white">No Connection Types</h3>
                                        <p class="mt-1 text-sm text-slate-500 dark:text-neutral-400">Get started by creating a new connection type.</p>
                                        <a href="{{ route('admin.connections.create') }}" wire:navigate class="mt-6 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:bg-emerald-600 dark:hover:bg-emerald-700">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
                                            Add Connection
                                        </a>
                                    </div>
            ...
            @elseif($connections->total() > 0)
                <div class="border-t border-slate-100 px-5 py-4 dark:border-neutral-800">
                    <p class="text-xs text-slate-400 dark:text-neutral-500">
                        Showing {{ $connections->firstItem() }} to {{ $connections->lastItem() }} of {{ $connections->total() }} results
                    </p>
                </div>
            @endif
        </div>
