<x-layouts.app>
    <x-slot:title>Edit Wholesaler - {{ $user->name }} - BWT</x-slot:title>
    <x-breadcrumb :items="[
        ['label' => 'Admin Poral', 'url' => route('admin.dashboard')],
        ['label' => 'Manage Wholeseller', 'url' => route('admin.wholeseller.index')],
        ['label' => $user->name],
    ]" />
    ...
    @php
        ...
        $approvedCount = $user->orders->where('status', 'approved')->count();
    @endphp

    <div class="space-y-6">
        {{-- Header --}}
        <div
            class="relative overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-6 shadow-sm dark:border-neutral-800 dark:from-neutral-900 dark:to-neutral-950">
            <div
                class="absolute right-0 top-0 h-32 w-64 translate-x-8 -translate-y-8 rounded-full bg-emerald-500/5 blur-3xl dark:bg-emerald-500/10">
            </div>
            <div class="relative flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div class="flex items-center gap-4">
                    <div
                        class="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 dark:bg-neutral-800 dark:ring-neutral-700">
                        {!! Avatar::create($user->name)->setDimension(64)->setFontSize(24)->toSvg() !!}
                    </div>
                    <div>
                        <div class="flex items-center gap-3">
                            <h1 class="text-2xl font-bold tracking-tight text-slate-950 dark:text-neutral-100">
                                {{ $user->name }}</h1>
                            <span
                                class="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">
                                <span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
                                Active
                            </span>
                        </div>
                        <p class="mt-1 text-sm text-slate-500 dark:text-neutral-400">
                            {{ $meta['wholesale_company_name'] ?? 'No company' }} &middot; {{ $user->email }}</p>
                        <div class="mt-3 flex flex-wrap items-center gap-3">
                            <span
                                class="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600 dark:bg-neutral-800 dark:text-neutral-400">
                ...
                <a href="{{ route('admin.wholeseller.edit', $user) }}"
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div
                    ...
                    <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-4 dark:border-neutral-800">
                        <div
                            class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/30 dark:to-amber-900/20">
                    ...
                    <div class="px-6 py-5">
                        <dl class="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
                            @foreach ([['Company Name', $meta['wholesale_company_name'] ?? '—', true], ['VAT Number', $meta['vat_number'] ?? '—', true], ['Address', $meta['address'] ?? '—', false], ['Postal Code', $meta['postal_code'] ?? '—', false], ['City', $meta['city'] ?? '—', false], ['Country', $meta['country'] ?? '—', false]] as [$label, $val, $semibold])
                                <div>
                                    <dt
                                        class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-neutral-500">
                                        {{ $label }}</dt>
                                    <dd
                                        class="mt-1 text-sm {{ $semibold ? 'font-semibold text-slate-900' : 'text-slate-700' }} dark:text-neutral-100">
                                        {{ $val }}</dd>
                                </div>
                            @endforeach
                ...
                <div
                    ...
                    <div class="flex items-center gap-3 border-b border-slate-100 px-6 py-4 dark:border-neutral-800">
                        <div
                            class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-900/20">
                        ...
                        <dl class="grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2">
                            <div>
                                <dt
                                    class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-neutral-500">
                                    Full Name</dt>
                                <dd class="mt-1 text-sm font-semibold text-slate-900 dark:text-neutral-100">
                                    {{ $user->name }}</dd>
                            </div>
                            <div>
                                <dt
                            ...
                            <div>
                                <dt
                                    class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-neutral-500">
                                    Phone</dt>
                                <dd class="mt-1 text-sm text-slate-700 dark:text-neutral-100">{{ $user->phone ?? '—' }}
                                </dd>
                            </div>
                            <div>
                                <dt
                                    class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-neutral-500">
                                    Username</dt>
                                <dd class="mt-1 text-sm font-mono font-medium text-slate-900 dark:text-neutral-100">
                                    {{ $user->username }}</dd>
                            </div>
                            <div class="md:col-span-2">
                                @php
                                    $decryptedPass = \App\Helpers\PasswordHelper::isEncrypted($meta['plain_password'])
                                        ...
                                        : $meta['plain_password'];
                                @endphp
                                <x-forms.password name="password" label="Password" :value="$decryptedPass" readonly
                                    :showGenerator="false" :showToggle="true" :showCopy="true" wrapperClass="w-full" />
                            </div>
                            <div>
                                <dt
                                    class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-neutral-500">
                                ...
                                <dd class="mt-1">
                                    <span
                                        class="inline-flex items-center rounded-full bg-emerald-100 px-3 py-1 text-xs font-medium text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">
                                        {{ 'Wholesaler' }}
                                    </span>
                                </dd>
                            </div>
                            <div>
                                ...
                                <dd class="mt-1">
                                    <span
                                        class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300">
                                        <span class="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
                                        Active
                                    </span>
                                </dd>
                            ...
                            <div>
                                <dt
                                    class="text-xs font-medium uppercase tracking-wider text-slate-400 dark:text-neutral-500">
                                    Member Since</dt>
                                <dd class="mt-1 text-sm text-slate-700 dark:text-neutral-100">
            ...
            <div class="space-y-6">
                ...
                <div
                    ...
                    <div class="flex items-center gap-2.5 mb-4">
                        <div
                            class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-sky-50 to-sky-100 dark:from-sky-900/30 dark:to-sky-900/20">
                            <svg class="h-4 w-4 text-sky-600 dark:text-sky-400" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    @if ($logoUrl)
                        <img src="{{ $logoUrl }}" alt="{{ $meta['wholesale_company_name'] ?? 'Logo' }}"
                            class="max-h-32 w-full rounded-xl border border-slate-100 object-contain p-4 dark:border-neutral-700">
                    @else
                        <div
                            class="flex h-32 items-center justify-center rounded-xl border-2 border-dashed border-slate-200 dark:border-neutral-700">
                            <div class="text-center">
                                <svg class="mx-auto h-8 w-8 text-slate-300 dark:text-neutral-600" fill="none"
                ...
                <div
                    ...
                    <div class="flex items-center gap-4 rounded-xl bg-slate-50 p-4 dark:bg-neutral-800">
                        <div
                            class="flex h-14 w-14 items-center justify-center rounded-xl bg-white shadow-sm dark:bg-neutral-800/80">
