<x-layouts.app>
    <x-slot:title>Wholesaler - BWT</x-slot:title>
    <x-breadcrumb :items="[['label' => 'Admin Portal', 'url' => route('admin.dashboard')], ['label' => 'Manage Wholesalers']]" />

    <div class="space-y-6">
        <x-ui.hero title="Manage Wholesalers" subtitle="Manage your Wholesaler accounts">
            <x-slot:actions>
                <a href="{{ route('admin.wholeseller.create') }}" wire:navigate
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <div
                class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 dark:border-neutral-800">
                <form method="GET" action="{{ route('admin.wholeseller.index') }}"
                    ...
                    <select name="role"
                        ...
                        <option value="Wholesaler" {{ request('role') === 'Wholesaler' ? 'selected' : '' }}>Wholesaler
                        </option>
                        <option value="Reseller" {{ request('role') === 'Reseller' ? 'selected' : '' }}>Reseller
                        </option>
                    ...
                    @if (request('search') || request('role'))
                        <a href="{{ route('admin.wholeseller.index') }}"
                            class="text-sm font-medium text-slate-500 hover:text-slate-700 dark:text-neutral-400">Clear</a>
                    @endif
                </form>
            </div>
            ...
            @php
                $roleConfig = [
                    'Admin' => [
                        ...
                        'pill' =>
                            'bg-purple-100 text-purple-800 ring-purple-200 dark:bg-purple-900/40 dark:text-purple-300 dark:ring-purple-700',
                    ],
                    'Wholesaler' => [
                        'icon' =>
                        ...
                        'pill' =>
                            'bg-emerald-100 text-emerald-800 ring-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-300 dark:ring-emerald-700',
                    ],
                    'Reseller' => [
                        'icon' =>
                        ...
                        'pill' =>
                            'bg-blue-100 text-blue-800 ring-blue-200 dark:bg-blue-900/40 dark:text-blue-300 dark:ring-blue-700',
                    ],
                    'customer' => [
                        'icon' =>
                            'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z',
                ...
                $roleOrder = ['Admin', 'Wholesaler', 'Reseller', 'customer'];
            ...
            @forelse($grouped as $roleName => $roleUsers)
                @php $cfg = $roleConfig[$roleName] ?? $roleConfig['Wholesaler']; @endphp
                <div class="border-b border-slate-100 last:border-b-0 dark:border-neutral-800">
                    <div class="flex items-center gap-3 px-5 py-4 {{ $cfg['bg'] }}">
                        ...
                        <span
                            class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {{ $cfg['pill'] }} ring-1">{{ $roleUsers->count() }}</span>
                    </div>
                    <div class="divide-y divide-slate-100 dark:divide-neutral-800">
                        @foreach ($roleUsers as $user)
                            @php $meta = $user->userMeta?->metadata ?? []; @endphp
                            <div
                                class="flex items-center gap-4 px-5 py-4 transition-colors hover:bg-slate-50/80 dark:hover:bg-neutral-800/30">
                                <div
                                    class="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-slate-100 ring-1 ring-slate-200 dark:bg-neutral-800 dark:ring-neutral-700">
                                ...
                                <div class="flex min-w-0 flex-1 items-center gap-4">
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center gap-2">
                                            @if ($roleName === 'Wholesaler')
                                                <a href="{{ route('admin.wholeseller.show', $user) }}" wire:navigate
                                    ...
                                    <div class="hidden sm:block min-w-0 flex-1">
                                        <p class="truncate text-sm text-slate-700 dark:text-neutral-300">
                                            {{ $user->phone ?? '—' }}</p>
                                    </div>
                                    <div class="hidden md:block min-w-0 flex-1">
                                        @if (!empty($meta['wholesale_company_name']))
                                            <p
                                                class="truncate text-sm font-medium text-slate-700 dark:text-neutral-300">
                                                {{ $meta['wholesale_company_name'] }}</p>
                                        @else
                                    ...
                                    <div class="hidden lg:flex flex-wrap gap-1">
                                        @forelse($user->roles as $role)
                                            <span
                                                class="inline-flex items-center rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700 ring-1 ring-inset ring-slate-200 dark:bg-neutral-800 dark:text-neutral-300 dark:ring-neutral-700">{{ $role->name }}</span>
                                        @empty
                                ...
                                <div class="flex shrink-0 items-center gap-1">
                                    ...
                                    @if ($roleName === 'Wholesaler')
                                        <a href="{{ route('admin.wholeseller.edit', $user) }}" wire:navigate
                                            title="Edit"
                                        ...
                                        <form action="{{ route('admin.wholeseller.destroy', $user) }}" method="POST"
                                            class="inline"
                ...
                @empty
                    <div class="flex flex-col items-center justify-center px-5 py-16 text-center">
                        <svg class="h-12 w-12 text-slate-300 dark:text-neutral-600" fill="none" viewBox="0 0 24 24"
                        ...
                        <p class="mt-1 text-sm text-slate-500 dark:text-neutral-400">Get started by creating a new user.
                        </p>
                        <a href="{{ route('admin.wholeseller.create') }}"
                            class="mt-6 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:bg-emerald-600 dark:hover:bg-emerald-700">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                            </svg>
