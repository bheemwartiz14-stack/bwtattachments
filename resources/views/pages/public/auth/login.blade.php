<x-guest-layout>
    <div class="flex min-h-screen items-center justify-center p-4">
        <div class="w-full max-w-md">
            <div
                class="overflow-hidden rounded-2xl border border-slate-100 bg-white p-8 shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
                <div class="mb-8 text-center">
                    <div class="mb-4 flex justify-center">
                        <img src="{{ asset('images/bwt-logo.jpg') }}" alt="BWT" class="h-10 w-auto">
                    </div>
                ...
                @if ($errors->any())
                    <div
                        class="mb-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-900/30 dark:text-red-300">
                ...
                @if (session('status'))
                    <div
                        class="mb-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-900/30 dark:text-emerald-300">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('login') }}" id="loginForm" class="space-y-5" novalidate>
                    ...
                    <div>
                        <label for="login"
                            class="block text-sm font-semibold text-slate-700 dark:text-neutral-300">Email or
                            username</label>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label for="remember_me" class="text-sm text-slate-600 dark:text-neutral-400">Remember
                            me</label>
                    </div>

                    <x-ui.button type="submit" variant="submit" block label="Sign in"
                        icon='<svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6" stroke-linecap="round" stroke-linejoin="round"/></svg>' />
                </form>
