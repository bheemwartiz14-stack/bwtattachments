        ...
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 gap-8 lg:grid-cols-5">
                <div class="space-y-6 lg:col-span-3">
                    ...
                    @endphp
                    <div class="w-full max-w-full overflow-hidden rounded-2xl bg-white shadow-sm"
                         x-data="{ current: 0, images: {{ json_encode($allImages) }} }">
                        <div class="relative aspect-[15/10] w-full max-w-full overflow-hidden bg-slate-100">
                            <template x-if="images.length">
                                <template x-for="(img, i) in images" :key="i">
                                    <img x-show="current === i"
                                         ...
                                         class="absolute inset-0 block h-full w-full max-w-full border-0 object-cover outline-none">
                                </template>
                            </template>
                            <template x-if="!images.length">
                                <div class="absolute inset-0 flex items-center justify-center">
                                    <div class="text-center">
                                        <svg class="mx-auto h-16 w-16 text-slate-300 sm:h-20 sm:w-20" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
                            ...
                            <template x-if="images.length > 1">
                                <div class="absolute inset-0 z-10">
                                    <button @click="current = current > 0 ? current - 1 : images.length - 1"
                                            class="absolute left-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 shadow-md transition-all hover:bg-white sm:left-3 sm:h-10 sm:w-10">
                                        <svg class="h-4 w-4 text-slate-700 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" /></svg>
                                    </button>
                                    <button @click="current = current < images.length - 1 ? current + 1 : 0"
                                            class="absolute right-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 shadow-md transition-all hover:bg-white sm:right-3 sm:h-10 sm:w-10">
                                        <svg class="h-4 w-4 text-slate-700 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" /></svg>
                                    </button>
                                    <div class="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5 sm:bottom-4">
                                        <template x-for="(img, i) in images" :key="i">
                        ...
                        <template x-if="images.length > 1">
                            <div class="flex w-full max-w-full gap-2 overflow-x-auto px-3 pb-4 pt-2 scrollbar-thin sm:px-4">
                                <template x-for="(img, i) in images" :key="i">
                                    <button @click="current = i"
                                            :class="current === i ? 'ring-2 ring-emerald-500 opacity-100' : 'opacity-50 hover:opacity-80'"
                                            class="relative aspect-[15/10] w-16 flex-shrink-0 overflow-hidden rounded-lg bg-slate-100 transition-all cursor-pointer sm:w-20 md:w-24">
                                        <img :src="img.thumb || img.url" alt="" class="absolute inset-0 block h-full w-full border-0 object-cover outline-none">
                                    </button>
                                </template>
                            </div>
                        </template>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                        <h2 class="mb-4 text-xl font-bold tracking-tight text-slate-950 sm:text-2xl">Technical Specifications</h2>
                        @php
                            $specs = [
                        ...
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">
                            @foreach($specs as $field => $config)
                                ...
                                <div class="min-w-0 rounded-lg bg-slate-50 p-3">
                                    <p class="truncate text-xs text-slate-500">{{ $config['label'] }}</p>
                                    <p class="mt-0.5 break-words text-sm font-semibold text-slate-950">
                                        @if($value !== null && $value !== '')
                                            {{ $value }}
                                            @if($config['unit'])
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                        <h2 class="mb-3 text-xl font-bold tracking-tight text-slate-950 sm:text-2xl">Description</h2>
                        <p class="break-words text-sm leading-relaxed text-slate-700">
                ...
                <div class="space-y-6 lg:col-span-2">
                    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                        <p class="break-words font-mono text-xs font-medium text-emerald-600">{{ $product->product_code ?? $product->code ?? 'CODE-001' }}</p>
                        <h1 class="mt-1 break-words text-xl font-bold tracking-tight text-slate-950 sm:text-2xl">{{ $product->description ?? $product->title ?? 'Product Name' }}</h1>
                        ...
                        <div class="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-center sm:p-6">
                            ...
                            <a href="{{ route('login') }}" class="mt-4 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 text-sm font-bold text-white shadow-lg shadow-emerald-200 transition hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">
                                ...
                                <span class="truncate">Login to View Pricing</span>
                            </a>
                        </div>
                        ...
                        @auth
                            @if(auth()->user()->isAdmin())
                                <div class="mt-6 border-t border-slate-100 pt-6">
                                    ...
                                    <textarea class="block w-full max-w-full rounded-xl border-slate-200 bg-white px-4 py-3 text-sm placeholder-slate-400 shadow-sm transition-colors focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500" rows="3" placeholder="Add internal notes..."></textarea>
                    ...
                    @if(($product->documents ?? $product->files ?? false) || true)
                        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                            ...
                            <div class="space-y-2">
                                <a href="#" class="group flex min-w-0 items-center gap-3 rounded-lg p-3 transition-colors hover:bg-slate-50">
                                    <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-red-50">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="truncate text-sm font-medium text-slate-950 transition-colors group-hover:text-emerald-600">Datasheet_{{ $product->product_code ?? $product->code ?? 'product' }}.pdf</p>
