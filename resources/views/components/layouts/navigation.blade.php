...
<nav class="sticky top-0 z-40 border-b border-slate-200 bg-white shadow-sm no-print dark:border-neutral-800 dark:bg-neutral-950">
                ...
                <div class="relative">
                    <button type="button"
                        ...
                        <span class="hidden text-left md:block">
                            ...
                            <span class="block max-w-32 truncate text-xs text-slate-500 dark:text-neutral-400">{{ $user->email }}</span>
                        </span>
                    ...
                    <div id="user-menu"
                        ...
                        <a href="{{ $profileRoute }}" class="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-700 transition-colors hover:bg-slate-50 dark:text-neutral-300 dark:hover:bg-neutral-900">
                            @svg('heroicon-o-cog-6-tooth', 'h-4 w-4')
                            Profile Settings
                        </a>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-600 transition-colors hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20">
