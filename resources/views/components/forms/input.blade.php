...
<div class="{{ $wrapperClass }}">
    ...
    <div class="relative">
        ...
        <input
            ...
            {{ $usernameAttr }}
            {{ $selectAttr }}
            {{ $attributes->merge(['class' => 'block w-full rounded-lg border px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-neutral-500 dark:focus:ring-neutral-500 autofill:!bg-white autofill:!text-inherit dark:autofill:!bg-white dark:autofill:!text-black ' . ($hasError ? 'border-red-300 dark:border-red-400 text-red-900 dark:text-red-800 placeholder-red-300 dark:placeholder-red-400 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 dark:border-gray-300 bg-white dark:bg-white text-gray-900 dark:text-gray-900 placeholder-gray-400 dark:placeholder-gray-400 focus:border-neutral-500') . ' ' . ($prepend ? 'pl-10' : '') . ' ' . ($append ? 'pr-10' : '') . ' ' . $class]) }}
        />
        @if($append)
