...
<div>
    ...
    @endif
    <div class="js-file-upload-dropzone"
        ...
        <div class="dropzone-area text-center cursor-pointer rounded-xl border-2 border-dashed p-8 transition-colors hover:border-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:hover:border-neutral-500 dark:focus:ring-emerald-400 border-slate-300 dark:border-neutral-700 {{ $hasExisting ? 'hidden' : '' }}"
            role="button" tabindex="0" aria-label="{{ $label ? $label . ': activate to choose a file' : 'Activate to choose a file' }}">
            ...
            <div class="flex items-center gap-4">
                <div class="shrink-0">
                    <img class="preview-img h-24 w-24 rounded-lg object-contain ring-1 ring-slate-200 dark:ring-neutral-700"
                        src="{{ $existingImageUrl ?? '' }}" alt="">
                    <div class="preview-filebadge hidden h-24 w-24 flex-col items-center justify-center rounded-lg bg-slate-100 ring-1 ring-slate-200 dark:bg-neutral-800 dark:ring-neutral-700"
                        aria-hidden="true">
                        <span class="preview-fileext text-xl font-extrabold uppercase tracking-wide text-slate-500 dark:text-neutral-300">AI</span>
                    </div>
                </div>
                <div class="min-w-0 flex-1">
                    <p class="preview-name truncate text-sm font-medium text-slate-700 dark:text-neutral-200"></p>
                    <p class="preview-size mt-0.5 text-xs text-slate-400 dark:text-neutral-500"></p>
                </div>
...
(function(){
    ...
    $('.js-file-upload-dropzone').each(function(){
        var $el = $(this);
        ...
        var $previewBadge = $el.find('.preview-filebadge');
        var $previewExt = $el.find('.preview-fileext');
        ...
        var dragBorder = $el.data('drag-border') || '#34d399';
        var dragBg = $el.data('drag-bg') || 'rgb(236 253 245)';
        ...
        var IMAGE_EXTS = ['jpg', 'jpeg', 'png', 'webp', 'gif', 'svg', 'bmp', 'ico'];

        function previewExtension(data) {
            if (data && data.extension) return String(data.extension).toLowerCase();
            var name = (data && data.name) || '';
            if (name.indexOf('.') !== -1) return name.split('.').pop().toLowerCase();
            var url = (existingUrl || '').split('?')[0];
            if (url.indexOf('.') !== -1) return url.split('.').pop().toLowerCase();
            return '';
        }

        function updatePreview(data) {
            var url = data && data.url ? data.url : existingUrl;
            var ext = previewExtension(data);
            if (IMAGE_EXTS.indexOf(ext) !== -1 && url) {
                $previewBadge.addClass('hidden').removeClass('flex');
                $previewImg.attr('src', url).removeClass('hidden');
            } else {
                $previewImg.addClass('hidden');
                $previewExt.text((ext || 'file').toUpperCase());
                $previewBadge.removeClass('hidden').addClass('flex');
            }
            if (data && data.name) {
                $previewName.text(data.name);
                $previewSize.text(formatSize(data.size));
            } else {
                $previewName.text('Current file');
                $previewSize.text('');
            }
        }
        ...
        // maxSize is supplied in BYTES (e.g. 10485760 = 10 MB).
        ...
        function handleFile(file) {
            setError('');
            if (!file) return;
            var type = (file.type || '').trim();
            var ext = ((file.name || '').split('.').pop() || '').toLowerCase();
            var extFilters = acceptedTypes
                .map(function(t) { return t.trim().toLowerCase(); })
                .filter(function(t) { return t.charAt(0) === '.'; })
                .map(function(t) { return t.slice(1); });
            var allowed;
            if (extFilters.length) {
                // Extension-based accept list (e.g. ".ai,.eps,..."): browsers report
                // empty/unreliable MIME types for vector formats, so match the extension.
                allowed = extFilters.indexOf(ext) !== -1;
            } else {
                allowed = acceptedTypes.some(function(t) {
            ...
            if (file.size > maxSize) {
                setError('File size must be less than ' + formatMaxSize(maxSize) + '.');
                return;
            }
            showView('uploading');
            $progressText.text('0%');
        ...
        $dropzone.on('dragover', function(e){
            e.preventDefault();
            $dropzone.css('border-color', dragBorder).css('background-color', dragBg);
        });
        ...
        $preview.on('dragover', function(e){
            e.preventDefault();
            $preview.css('border-color', dragBorder).css('background-color', dragBg);
        });
