    ...
    <ol class="inline-flex items-center gap-1.5 text-sm text-gray-400 dark:text-neutral-500">
        <li>
            <a href="{{  $dashboardRoute }}"
                class="transition-colors hover:text-gray-700 dark:hover:text-slate-300">
        ...
        @foreach ($items as $item)
            <li class="flex items-center gap-1.5">
                ...
                @if (isset($item['url']))
                    <a href="{{ $item['url'] }}"
                        class="transition-colors hover:text-gray-700 dark:hover:text-slate-300">{{ $item['label'] }}</a>
                @else
