    ...
    <div class="relative aspect-[15/10] overflow-hidden bg-slate-100 dark:bg-neutral-900">
        ...
        @auth
            <button type="button"
                ...
                data-favorited="{{ isset($favorited) && $favorited ? 'true' : 'false' }}"
                onclick="toggleFavorite(this)"
                class="absolute bottom-2 right-2 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white/80 shadow-sm backdrop-blur-sm transition-all hover:scale-110 dark:bg-neutral-900/80"
                title="{{ isset($favorited) && $favorited ? 'Remove from favorites' : 'Add to favorites' }}">
                <svg class="h-4 w-4 transition-colors {{ isset($favorited) && $favorited ? 'text-red-500 fill-red-500' : 'text-slate-500 dark:text-neutral-400' }}"
                    viewBox="0 0 24 24" fill="{{ isset($favorited) && $favorited ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="2">
