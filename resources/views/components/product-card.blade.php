<div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition-all duration-200 group hover:shadow-md dark:border-slate-700 dark:bg-slate-900">
    <div class="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-800">
        ...
        <div class="absolute top-2 left-2 flex flex-wrap gap-1">
            ...
            @if(isset($subcategory))
                <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-700 dark:bg-slate-700 dark:text-slate-300">{{ $subcategory }}</span>
            @endif
        </div>
    ...
    <div class="p-4">
        ...
        <h3 class="mt-1 line-clamp-2 text-sm font-semibold text-black dark:text-gray-100">{{ $title ?? 'Product Name' }}</h3>
        <div class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-700 dark:text-gray-400">
            @if(isset($weight))
        ...
        @if(isset($showPrice) && $showPrice)
            <div class="mt-3 border-t border-slate-100 pt-3 dark:border-slate-700">
                <p class="text-lg font-bold text-black dark:text-gray-100">{{ $price ?? '$0.00' }}</p>
            </div>
        @endif
