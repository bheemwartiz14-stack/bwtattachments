...
$rolePrefix = match (true) {
    $user->hasRole('Admin') => 'admin', $user->hasRole('Wholesaler') => 'client',
    $user->hasRole('Retailer') => 'retailer', $user->hasRole('customer') => 'customer',
    default => null,
};
if ($user->hasRole('Admin')) {
    $sidebarItems = [
        ['label' => 'Dashboard', 'route' => 'admin.dashboard', 'pattern' => 'admin.dashboard'],
        ['label' => 'Manage Wholesalers', 'route' => 'admin.wholeseller.index', 'pattern' => 'admin.wholeseller.*', 'icon' => 'heroicon-o-building-storefront'],
...
<aside data-sidebar class="hidden rounded-2xl border border-slate-200 bg-white p-3 shadow-sm no-print dark:border-neutral-800 dark:bg-neutral-950 lg:block">
        ...
        @foreach ($sidebarItems as $item)
            ...
            @if ($hasChildren)
                <div x-data="{ open: @json($isGroupActive) }" class="pt-1">
                    <button @click="open = !open" class="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition {{ $isGroupActive ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-neutral-400 dark:hover:bg-neutral-900 dark:hover:text-white' }}">
                        <span class="flex h-5 w-5 items-center justify-center">@svg($icon, 'h-5 w-5')</span>
                        <span class="flex-1 text-left">{{ $item['label'] }}</span>
                        <svg class="h-4 w-4 transition-transform duration-200" :class="{ 'rotate-180': open }" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>
                    </button>
                    <div x-show="open" x-collapse.duration.200ms>
                        <div class="ml-4 mt-0.5 space-y-0.5 border-l-2 border-slate-200 pl-2 dark:border-neutral-700">
                            @foreach ($item['children'] as $child)
                                ...
                                @endphp
                                <a href="{{ $href }}" wire:navigate class="group flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-all {{ $isChildActive ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-neutral-400 dark:hover:bg-neutral-900 dark:hover:text-white' }}">
                                    <span class="h-1.5 w-1.5 rounded-full {{ $isChildActive ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-neutral-600' }}"></span>
                                    {{ $child['label'] }}
                                </a>
                            @endforeach
                        </div>
            ...
            @else
                ...
                @endphp
                <a href="{{ $href }}" wire:navigate class="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all {{ $isActive ? 'bg-emerald-50 text-emerald-700 shadow-sm dark:bg-emerald-900/20 dark:text-emerald-400' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-neutral-400 dark:hover:bg-neutral-900 dark:hover:text-white' }}">
                    <span class="flex h-5 w-5 items-center justify-center">@svg($icon, 'h-5 w-5')</span>
                    <span class="flex-1">{{ $item['label'] }}</span>
                    @if ($isActive)<span class="h-2 w-2 rounded-full bg-emerald-500"></span>@endif
                </a>
            @endif
        @endforeach
    </nav>
