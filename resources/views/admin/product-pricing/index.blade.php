<x-layouts.app>
    <x-slot:title>Product Wholesale Pricing - Attachment Portal</x-slot:title>
    <x-breadcrumb :items="[['label' => 'Admin', 'url' => route('admin.dashboard')], ['label' => 'Product Pricing']]" />

    <div class="space-y-6">
        <x-ui.hero title="Product Wholesale Pricing" subtitle="Manage wholesale purchase prices for products">
            <x-slot:actions>
                <a href="{{ route('admin.product-pricing.create') }}"
        ...
        <form method="GET" action="{{ route('admin.product-pricing.index') }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                <div class="lg:col-span-2">
                    <label class="block text-xs font-medium text-slate-500 dark:text-neutral-400 mb-1">Search</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by product or user..." class="block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-black placeholder-slate-400 transition-colors focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100 dark:placeholder-neutral-500">
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-500 dark:text-neutral-400 mb-1">Price Type</label>
                    <select name="type" class="block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-black transition-colors focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100">
                        <option value="">All Types</option>
                        <option value="wholesale_purchase" @selected(request('type') === 'wholesale_purchase')>Wholesale Purchase</option>
                        <option value="wholesale_selling" @selected(request('type') === 'wholesale_selling')>Wholesale Selling</option>
                        <option value="retail" @selected(request('type') === 'retail')>Retail</option>
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-slate-500 dark:text-neutral-400 mb-1">Per Page</label>
                    <select name="per_page" class="block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-black transition-colors focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100">
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('admin.product-pricing.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-400 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 dark:hover:bg-neutral-800">
                        Clear
                    </a>
                </div>
            </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
            <div class="border-b border-slate-100 px-5 py-4 dark:border-neutral-800">
                <h2 class="text-base font-semibold text-slate-900 dark:text-white">All Product Prices</h2>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50/50 dark:border-neutral-800 dark:bg-neutral-900/50">
                            ...
                            <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-neutral-400">Type</th>
                            ...
                            <th class="px-5 py-3.5 text-right text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-neutral-400">Price</th>
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div class="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-slate-100 dark:bg-neutral-800">
                                            @if($price->product?->getFirstMediaUrl('images'))
                                                <img src="{{ $price->product->getFirstMediaUrl('images', 'thumb') }}" class="h-full w-full object-cover">
                                            @else
                                        ...
                                        <div>
                                            <a href="{{ route('admin.product-pricing.show', $price) }}" class="font-medium text-slate-900 hover:text-emerald-600 dark:text-white dark:hover:text-emerald-400">{{ $price->product?->product_title ?? 'N/A' }}</a>
                                            <p class="text-xs text-slate-400">{{ $price->product?->product_code ?? '' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-2.5">
                                        ...
                                        <span class="font-medium text-slate-900 dark:text-white">{{ $price->user?->name ?? 'N/A' }}</span>
                                    </div>
                                </td>
                                <td class="px-5 py-4">
                                    @php
                                        $typeClasses = [
                                            'wholesale_purchase' => 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300',
                                            'wholesale_selling' => 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300',
                                            'retail' => 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300',
                                        ];
                                        $typeLabels = [
                                            'wholesale_purchase' => 'Purchase',
                                            'wholesale_selling' => 'Selling',
                                            'retail' => 'Retail',
                                        ];
                                        $class = $typeClasses[$price->type] ?? 'bg-slate-100 text-slate-700';
                                        $label = $typeLabels[$price->type] ?? $price->type;
                                    @endphp
                                    <span class="inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $class }}">
                                        {{ $label }}
                                    </span>
                                </td>
                                <td class="px-5 py-4 text-slate-600 dark:text-neutral-400">{{ $price->user?->email ?? '' }}</td>
                                <td class="px-5 py-4 text-right font-semibold text-emerald-700 dark:text-emerald-400">{{ config('app.currency_symbol') }}{{ number_format($price->price, 2) }}</td>
                                <td class="px-5 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('admin.product-pricing.show', $price) }}" title="View" class="inline-flex items-center justify-center rounded-lg p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500 dark:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-white">
                                        ...
                                        <a href="{{ route('admin.product-pricing.edit', $price) }}" title="Edit" class="inline-flex items-center justify-center rounded-lg p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500 dark:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-white">
                            ...
                            <tr>
                                <td colspan="6">
                                    <div class="flex flex-col items-center justify-center px-5 py-16 text-center">
                                        ...
                                        <h3 class="mt-4 text-sm font-semibold text-slate-900 dark:text-white">No Prices Found</h3>
                                        <p class="mt-1 text-sm text-slate-500 dark:text-neutral-400">Get started by setting a price.</p>
                                        <a href="{{ route('admin.product-pricing.create') }}" class="mt-6 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-colors hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:bg-emerald-600 dark:hover:bg-emerald-700">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
                                            Add Price
                                        </a>
                                    </div>
