<x-layouts.app>
    <x-slot:title>Subcategories - Attachment Portal</x-slot:title>
    <x-breadcrumb :items="[['label' => 'Admin', 'url' => route('admin.dashboard')], ['label' => 'Subcategories']]" />

    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-950 dark:text-slate-100">Subcategories</h1>
                <p class="text-sm text-gray-700 mt-1">Manage product subcategories</p>
            </div>
            <a href="{{ route('admin.subcategories.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-rose-50 px-4 py-2 text-sm font-medium text-rose-700 transition-colors hover:bg-rose-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:bg-rose-900/30 dark:text-rose-300 dark:hover:bg-rose-900/50">
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse($subcategories ?? [] as $subcategory)
                            <tr class="hover:bg-rose-50 dark:bg-slate-800/50 dark:hover:bg-slate-800/50 transition-colors">
                                <td class="px-6 py-4 font-medium text-black dark:text-gray-100">{{ $subcategory->name }}</td>
                                <td class="px-6 py-4">
                                    @if($subcategory->category)
                                        <span class="inline-flex items-center rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-medium text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300">{{ $subcategory->category->name }}</span>
                                    @else
                                        <span class="text-xs text-gray-400">N/A</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-gray-700 dark:text-gray-400 font-mono text-xs">{{ $subcategory->slug }}</td>
                                <td class="px-6 py-4">
                                    @if(isset($subcategory->status) && $subcategory->status === 'inactive')
                                ...
                                <td class="px-6 py-4 text-gray-700 dark:text-gray-400">{{ $subcategory->created_at->format('Y-m-d') }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('admin.subcategories.edit', $subcategory) }}" title="Edit" class="inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-rose-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:text-gray-400 dark:hover:bg-slate-800">
            ...
            @if(isset($subcategories) && $subcategories instanceof \Illuminate\Pagination\LengthAwarePaginator && $subcategories->hasPages())
                <div class="px-6 py-3 border-t border-slate-100 dark:border-slate-700">
                    {{ $subcategories->withQueryString()->links() }}
                </div>
            @elseif(isset($subcategories) && $subcategories instanceof \Illuminate\Pagination\LengthAwarePaginator)
                <div class="px-6 py-3 border-t border-slate-100 dark:border-slate-700 flex items-center justify-between">
                    <p class="text-xs text-gray-400 dark:text-gray-500">Showing {{ $subcategories->firstItem() }} to {{ $subcategories->lastItem() }} of {{ $subcategories->total() }} results</p>
                </div>
            @endif
        </div>
    </div>
