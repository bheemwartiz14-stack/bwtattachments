<div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
    <div class="p-6">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 dark:border-neutral-800">
                        <th
                            class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 w-10">
                            #</th>
                        <th class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">
                            Product</th>
                        <th
                            class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 w-24">
                            Price</th>
                        <th
                            class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 w-16">
                            Qty</th>
                        <th
                            class="px-3 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-400 w-24">
                            Total</th>
                        <th
                            class="px-3 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-400 w-10">
                        </th>
                    </tr>
                </thead>
                        ...
                        <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-neutral-900/50">
                            ...
                            <td class="px-3 py-3">
                                ...
                                @if ($item['product_code'] ?? false)
                                    <p class="text-xs text-slate-400 dark:text-neutral-500 font-mono">
                                        {{ $item['product_code'] }}</p>
                                @endif
                            </td>
                            <td class="px-3 py-3">
                            ...
                            <td class="px-3 py-3">
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <span
                                        class="inline-flex h-7 min-w-[2rem] items-center justify-center rounded-lg border border-slate-200 bg-slate-50 px-2 text-sm font-semibold text-slate-900 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-3 text-sm font-medium text-slate-500 dark:text-neutral-400">No items added
                                    yet</p>
                                <p class="mt-1 text-xs text-slate-400 dark:text-neutral-500">Click "Add Item" to select
                                    products</p>
                            </td>
                        </tr>
                    @endforelse
        ...
        @if (count($items) > 0)
            <div
                class="mt-6 overflow-hidden rounded-xl border border-slate-100 bg-slate-50/70 dark:border-neutral-800 dark:bg-neutral-900/50">
                <div class="p-4 sm:p-5">
                    <div class="ml-auto max-w-sm space-y-3">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <span class="flex items-center gap-2 text-slate-500 dark:text-neutral-400">
                                <span class="h-1.5 w-1.5 rounded-full bg-amber-500"></span> VAT
                                ({{ $this->taxRate }}%)
                            </span>
                            <span
