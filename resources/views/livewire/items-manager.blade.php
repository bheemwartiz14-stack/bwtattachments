<div class="rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-950">
    <div class="p-6">
        <div class="mb-5 flex items-center justify-between border-b border-slate-100 pb-4 dark:border-neutral-800">
            <div class="flex items-center gap-2.5">
                <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-rose-100 dark:bg-rose-900/50">
                    <svg class="h-4 w-4 text-rose-600 dark:text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                </div>
                <div>
                    <h2 class="text-base font-semibold text-slate-900 dark:text-white">Items</h2>
                    <p class="text-sm text-gray-500 dark:text-neutral-400">Add products to your quotation</p>
                </div>
            </div>
            <button type="button" wire:click="openModal"
                class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition-all hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:hover:bg-emerald-500">
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full sm:max-w-xl rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="px-6 py-3 border-b border-slate-100 dark:border-neutral-800">
                    <input type="text" wire:model.live.debounce.200ms="search" placeholder="Search products..."
                        class="block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 shadow-sm transition-colors focus:border-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-500 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100 dark:placeholder-neutral-500">
                </div>
                <div class="max-h-80 overflow-y-auto">
                    @forelse($products as $product)
                        ...
                        @endphp
                        <button type="button"
                            @if($added) disabled class="flex w-full items-center gap-4 px-6 py-3.5 text-left text-sm opacity-50 cursor-default border-b border-slate-50 dark:border-neutral-800"
                            @else wire:click="addItem('{{ $product->id }}')" class="flex w-full items-center gap-4 px-6 py-3.5 text-left text-sm transition-colors hover:bg-slate-50 dark:hover:bg-neutral-800 border-b border-slate-50 dark:border-neutral-800"
                            @endif>
                            <div class="flex-1 min-w-0">
                                <p class="font-medium text-slate-900 dark:text-white">{{ $product->product_title }}</p>
                                <p class="text-xs text-slate-400 dark:text-neutral-500 mt-0.5">{{ $product->product_code ?? '' }}{{ $product->category ? ' · ' . $product->category->name : '' }}</p>
                            </div>
                            <span class="text-sm font-semibold text-slate-900 dark:text-white">&euro;{{ number_format($price, 2) }}</span>
                            <span class="inline-flex items-center rounded-lg px-3 py-1.5 text-xs font-medium {{ $added ? 'bg-slate-100 text-slate-500 dark:bg-neutral-800 dark:text-neutral-400' : 'bg-emerald-600 text-white' }}">
                                {{ $added ? 'Added' : 'Add' }}
                            </span>
                        </button>
                    @empty
